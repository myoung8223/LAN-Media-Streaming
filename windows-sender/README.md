...
Install the free .NET 8 SDK from https://dotnet.microsoft.com/download
(no Visual Studio required).
In this folder, produce a self-contained single `.exe` (no runtime needed on
the target PC):
```powershell
...
---
Use it
On the Android panel, open LAN Media Receiver, note its IP and
port, set a password if you want, and tap Start listening.
Launch LAN Media Sender on the PC. Enter the panel's IP, the same
port and password, choose System audio or Microphone, and
click Start streaming.
The level bar shows audio is flowing and the status reads "● Streaming".
