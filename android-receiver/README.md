...
1. Install **Android Studio** (latest stable) from developer.android.com. During
   first launch let it install the Android SDK it recommends.
2. **Open** this folder (`lan-audio-receiver`) in Android Studio: *File → Open →*
...
3. Build the installable APK: **Build → Build Bundle(s) / APK(s) → Build APK(s)**.
...
1. On the panel, allow installing from unknown sources (Settings → Security, or
   the per-app "Install unknown apps" permission for your file manager).
2. Copy `app-debug.apk` to the panel (USB drive, network share, or
   `adb install app-debug.apk`) and open it to install.
3. Launch **LAN Media Receiver**. It shows the panel's **IP address** and a
...
4. **Minimize** sends the app to the background while it keeps playing.
5. **Start automatically after reboot** (toggle) resumes listening after the panel
   restarts — see the note below.
6. **Important for always-on use:** tap **Open app settings** and exempt the app
...
- The tiny handshake + raw-PCM format is in [`PROTOCOL.md`](PROTOCOL.md).
